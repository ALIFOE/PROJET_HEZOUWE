<template>
    <AppLayout
        title="Témoignages"
        description="Ce que disent les clients et partenaires de COOP CA HEZOUWE sur la qualité du riz local togolais et les services de la coopérative."
    >
        <!-- Breadcrumb Section Start -->
        <section class="breadcrumb-wrapper bg-cover fix" style="background-image: url('/assets/img/inner-page/breadcroumb.jpg');">
            <div class="shape-1 float-bob-y">
                <img src="/assets/img/inner-page/shape-1.png" alt="img">
            </div>
            <div class="shape-2 float-bob-x">
                <img src="/assets/img/inner-page/shape-2.png" alt="img">
            </div>
            <div class="container">
                <div class="page-heading">
                    <ul class="breadcrumb-list wow fadeInUp">
                        <li><Link href="/">Accueil</Link></li>
                        <li>//</li>
                        <li>Témoignages</li>
                    </ul>
                    <h1 class="breadcrumb-title text-anim">Témoignages Clients</h1>
                </div>
            </div>
        </section>

        <!-- Testimonial Section Start -->
        <section class="testimonial-section section-padding fix">
            <div class="container">
                <div class="row g-4">
                    <div class="col-lg-6 wow fadeInUp" data-wow-delay=".2s">
                        <div class="testimonial-box-items-inner mt-0">
                            <div class="star">
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                            </div>
                            <h4>"Le meilleur riz blanc que j'ai jamais goûté! La qualité est remarquable et le service est excellent. Je recommande HEZOUWE à  tous mes amis!"</h4>
                            <div class="client-info-area">
                                <div class="client-info">
                                    <div class="client-image">
                                        <img src="/assets/img/home-1/testimonial/client-image.png" alt="img">
                                    </div>
                                    <div class="content">
                                        <p>Cliente Fidèle</p>
                                        <h5>Adèle Mensah</h5>
                                    </div>
                                </div>
                                <div class="icon">
                                    <i class="flaticon-quote"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 wow fadeInUp" data-wow-delay=".4s">
                        <div class="testimonial-box-items-inner mt-0">
                            <div class="star">
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                            </div>
                            <h4>"HEZOUWE offre un prix juste et un produit de haute qualité. Je suis très heureux de soutenir une coopérative locale qui travaille avec nos producteurs."</h4>
                            <div class="client-info-area">
                                <div class="client-info">
                                    <div class="client-image">
                                        <img src="/assets/img/home-1/testimonial/client-image.png" alt="img">
                                    </div>
                                    <div class="content">
                                        <p>Entrepreneur</p>
                                        <h5>Komlavi Appoh</h5>
                                    </div>
                                </div>
                                <div class="icon">
                                    <i class="flaticon-quote"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 wow fadeInUp" data-wow-delay=".6s">
                        <div class="testimonial-box-items-inner mt-0">
                            <div class="star">
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                            </div>
                            <h4>"J'utilise le riz HEZOUWE pour mon restaurant. C'est du riz blanc pur, sans mélange, avec une saveur naturelle authentique. Mes clients adorent!"</h4>
                            <div class="client-info-area">
                                <div class="client-info">
                                    <div class="client-image">
                                        <img src="/assets/img/home-1/testimonial/client-image.png" alt="img">
                                    </div>
                                    <div class="content">
                                        <p>Restaurateur</p>
                                        <h5>Yao Tawiah</h5>
                                    </div>
                                </div>
                                <div class="icon">
                                    <i class="flaticon-quote"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 wow fadeInUp" data-wow-delay=".8s">
                        <div class="testimonial-box-items-inner mt-0">
                            <div class="star">
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                            </div>
                            <h4>"La certification ITRA garantit la qualité. Je suis confiant en recommandant HEZOUWE à  ma famille et mes collègues. Service rapide et fiable!"</h4>
                            <div class="client-info-area">
                                <div class="client-info">
                                    <div class="client-image">
                                        <img src="/assets/img/home-1/testimonial/client-image.png" alt="img">
                                    </div>
                                    <div class="content">
                                        <p>Acheteur Professionnel</p>
                                        <h5>Assane Diallo</h5>
                                    </div>
                                </div>
                                <div class="icon">
                                    <i class="flaticon-quote"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 wow fadeInUp" data-wow-delay=".2s">
                        <div class="testimonial-box-items-inner mt-0">
                            <div class="star">
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                            </div>
                            <h4>"En tant que producteur partenaire, je suis fier de travailler avec HEZOUWE. Ils valorisent notre travail et offrent une rémunération équitable."</h4>
                            <div class="client-info-area">
                                <div class="client-info">
                                    <div class="client-image">
                                        <img src="/assets/img/home-1/testimonial/client-image.png" alt="img">
                                    </div>
                                    <div class="content">
                                        <p>Producteur Riz</p>
                                        <h5>Alassane Sow</h5>
                                    </div>
                                </div>
                                <div class="icon">
                                    <i class="flaticon-quote"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6 wow fadeInUp" data-wow-delay=".4s">
                        <div class="testimonial-box-items-inner mt-0">
                            <div class="star">
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                                <i class="fas fa-star"></i>
                            </div>
                            <h4>"HEZOUWE m'a aidée à  transformer ma récolte correctement. L'équipe technique est très professionnelle et toujours disponible pour conseiller."</h4>
                            <div class="client-info-area">
                                <div class="client-info">
                                    <div class="client-image">
                                        <img src="/assets/img/home-1/testimonial/client-image.png" alt="img">
                                    </div>
                                    <div class="content">
                                        <p>Productrice</p>
                                        <h5>Awa Coulibaly</h5>
                                    </div>
                                </div>
                                <div class="icon">
                                    <i class="flaticon-quote"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="page-nav-wrap text-center">
                    <ul>
                        <li class="active"><a class="page-numbers" href="javascript:void(0)"><i class="fal fa-long-arrow-left"></i></a></li>
                        <li><a class="page-numbers" href="javascript:void(0)">01</a></li>
                        <li><a class="page-numbers" href="javascript:void(0)">02</a></li>
                        <li class="active"><a class="page-numbers" href="javascript:void(0)"><i class="fal fa-long-arrow-right"></i></a></li>
                    </ul>
                </div>
            </div>
        </section>

       <!-- Cta Discount Section Start -->
        <section class="cta-discount-section-4 section-padding pt-0">
            <div class="container">
                <div class="cta-wrapper-4 bg-cover" style="background-image: url('/assets/img/home-4/cta-discount-bg.jpg');">
                    <div class="cta-discount-area">
                        <div class="circle-bg-icon wow fadeInUp">
                            <img src="/assets/img/home-3/cta-discount/white-circle.png" alt="img" class="circle-icon">
                            <div class="icon">
                                <img src="/assets/img/home-4/icon-01.svg" alt="img">
                            </div>
                        </div>
                        <div class="section-title wow fadeInUp" data-wow-delay=".3s">
                            <span class="text-white"><img src="/assets/img/sub-title.svg" alt="img">Offre Spéciale</span>
                            <h2 class="text-white text-anim">Profitez de Réductions <br> Jusqu'à  50% Actuellement!</h2>
                        </div>
                    </div>
                    <Link href="/about" class="theme-btn wow fadeInUp" data-wow-delay=".7s">
                       Découvrir Plus <i class="far fa-arrow-right"></i>
                    </Link>
                </div>
            </div>
        </section>

        <!-- Gallery Section Start -->
        <div class="gallery-section-4 section-padding fix pt-0 section-bg">
            <div class="swiper gallery-slide">
                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <div class="gallery-image-4">
                            <img src="/assets/img/image1.jpg" alt="img">
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="gallery-image-4">
                            <img src="/assets/img/image2.jpg" alt="img">
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="gallery-image-4">
                            <img src="/assets/img/image3.jpg" alt="img">
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="gallery-image-4">
                            <img src="/assets/img/image4.jpg" alt="img">
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="gallery-image-4">
                            <img src="/assets/img/image5.jpg" alt="img">
                        </div>
                    </div>
                    <div class="swiper-slide">
                        <div class="gallery-image-4">
                            <img src="/assets/img/home-4/gallery/gallery-06.jpg" alt="img">
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </AppLayout>
</template>

<script setup>
import AppLayout from '@/Layouts/AppLayout.vue';
import { Link } from '@inertiajs/vue3';
import { onMounted } from 'vue';

onMounted(() => {
    if (typeof Swiper !== 'undefined') {
        // Initialize gallery slider
        new Swiper('.gallery-slide', {
            loop: true,
            slidesPerView: 5,
            spaceBetween: 30,
            autoplay: {
                delay: 3000,
            },
            breakpoints: {
                1200: {
                    slidesPerView: 5,
                },
                992: {
                    slidesPerView: 4,
                },
                768: {
                    slidesPerView: 3,
                },
                576: {
                    slidesPerView: 2,
                },
                0: {
                    slidesPerView: 1,
                },
            },
        });
    }
});
</script>
